                                  ...
                                  const std::vector<float>& embedding, float quality, qint64 mtime)
{
    ...
    QString globalId = assignOrFindGlobalID(embedding);
...
QString FaceDatabaseManager::assignOrFindGlobalID(const std::vector<float>& embedding) {
    ...
    if (query.exec()) {
        while (query.next()) {
            ...
            if (dist < 0.5f) {
                matchedId = QString("f_%1").arg(existingId, 5, 10, QLatin1Char('0'));
    ...
    if (matchedId.isEmpty()) {
        ...
        if (insert.exec()) {
            matchedId = QString("f_%1").arg(insert.lastInsertId().toInt(), 5, 10, QLatin1Char('0'));
        } else {
