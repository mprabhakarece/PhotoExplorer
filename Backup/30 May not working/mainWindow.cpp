...
QPixmap getCachedThumbnail(const QString &imagePath, QSize size) {
    ...
    QDir cacheDir(virtualCachePath(info.absolutePath()) + "/thumbnails");
    ...
    QFuture<void> future = QtConcurrent::run([this]() {
        ...
        while (it.hasNext()) {
            ...
            for (const QRect& rect : faces) {
                ...
                if (!matched) {
                    personList.push_back(FaceStats{
                        .embedding = embedding,
                        .symmetry = symmetry,
                        .focus = focus,
                        .thumb = thumb,
            ...
            for (int i = 0; i < static_cast<int>(faceRects.size()); ++i) {
                faceIndexer.saveFaceEntry(path, faceRects[i], embeddings[i], focuses[i], QFileInfo(path).lastModified().toSecsSinceEpoch());
            }
        }
...
void MainWindow::updateFaceList() {
    ...
    QList<FaceEntry> entries = faceIndexer.getFaceEntriesInFolder(currentPath);
    QMap<QString, QList<FaceEntry>> grouped;

    // Group by globalId
    for (const FaceEntry& entry : entries) {
        grouped[entry.globalId].append(entry);
    }

    for (auto it = grouped.begin(); it != grouped.end(); ++it) {
        const QList<FaceEntry>& faces = it.value();
        if (faces.isEmpty()) continue;

        // Pick best (or first) face in group
        const FaceEntry& rep = *std::max_element(faces.begin(), faces.end(),
                                                 [](const FaceEntry& a, const FaceEntry& b) {
                                                     return a.quality < b.quality;
                                                 });

        QImageReader reader(rep.imagePath);
        reader.setAutoTransform(true);
        QImage fullImage = reader.read();

        if (fullImage.isNull()) continue;

        QRect rect = rep.faceRect;
        rect &= QRect(0, 0, fullImage.width(), fullImage.height());  // safety clamp

        QImage cropped = fullImage.copy(rect);
        QPixmap thumb = QPixmap::fromImage(
            cropped.scaled(84, 84, Qt::KeepAspectRatio, Qt::SmoothTransformation)
            );

        FaceStats stats;
        stats.imagePath = rep.imagePath;
        stats.focus = rep.quality;
        stats.thumb = thumb;
        stats.count = faces.size();
        stats.embedding = FaceDatabaseManager::instance().getEmbeddingById(rep.id);
        personList.push_back(stats);

        QString label = QString("Person %1 (%2)").arg(personList.size()).arg(stats.count);
        QListWidgetItem* item = new QListWidgetItem(QIcon(thumb), label);
        item->setToolTip(rep.imagePath);
        item->setFlags(item->flags() | Qt::ItemIsUserCheckable);
        item->setCheckState(Qt::Unchecked);
        faceList->addItem(item);
    }
...
void MainWindow::updateFolderViewCheckboxesFromFaceSelection() {
    ...
    for (int i = 0; i < folderView->count(); ++i) {
        ...
        QList<FaceEntry> faces = faceIndexer.getFaceEntriesInFolder(info.absolutePath());
