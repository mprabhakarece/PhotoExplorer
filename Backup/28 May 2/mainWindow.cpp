    ...
    QtConcurrent::run([this]() {
        ...
        while (it.hasNext()) {
            ...
            for (const QRect& rect : faces) {
                ...
                if (rect.width() < 40 || rect.height() < 40) continue;  // Skip tiny
                if (rect.width() > 1000 || rect.height() > 1000) continue;  // Skip huge
                ...
                // Scale face rectangle back to original image
                QRect scaled = QRect(
                    int(rect.x() * scaleX),
                    int(rect.y() * scaleY),
                ...
                for (size_t i = 0; i < personList.size(); ++i) {
                    if (isSimilarFace(embedding, personList[i].embedding, matchDIST)) {
                        matched = true;
                        personList[i].count += 1;  // ✅ Always increment, even if thumbnail isn't updated
                        ...
                        if (eyesAreOpen(shape)) {
                            ...
                            if (symmetry < personList[i].symmetry && (focusGood || focusAcceptable)) {
                                qDebug() << "🔁 Better face for person" << i << "Sym:" << symmetry << "Focus:" << focus;
                                ...
                                qDebug() << "✅ Match found, not better. Person:" << i << "Sym:" << symmetry;
                                // ✅ Still update count label even without thumbnail change
                                QMetaObject::invokeMethod(this, [=]() {
                                    faceList->item(static_cast<int>(i))->setText(QString("Person %1 (%2)").arg(i + 1).arg(personList[i].count));
                                }, Qt::QueuedConnection);
                            }
                        } else {
                            qDebug() << "🚫 Eyes closed, skipping thumbnail. Still counted as match.";
                            QMetaObject::invokeMethod(this, [=]() {
                                faceList->item(static_cast<int>(i))->setText(QString("Person %1 (%2)").arg(i + 1).arg(personList[i].count));
                            }, Qt::QueuedConnection);
                        }
                ...
                if (!matched) {
                    qDebug() << "➕ New person from:" << path << "Symmetry:" << symmetry << "Focus:" << focus;
        ...
        QMetaObject::invokeMethod(this, [this]() {
            // Step 1: Sort personList by count descending
            std::sort(personList.begin(), personList.end(), [](const FaceStats& a, const FaceStats& b) {
                return a.count > b.count;
            });

            // Step 2: Clear and rebuild faceList fully
            faceList->clear();
