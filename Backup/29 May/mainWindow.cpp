...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
    ...
    connect(pasteButton, &QPushButton::clicked, this, [this]() {
        ...
        // ✅ Remove dummy.jpg if present
        QString dummyPath = currentPath + "/dummy.jpg";
        if (QFile::exists(dummyPath)) QFile::remove(dummyPath);
        ...
        for (const QString& src : copiedFilePaths) {
            ...
            if (QFile::exists(destPath)) {
                ...
                static bool userChoseOverwrite = false;  // stores user's first choice
        ...
        statusBar()->showMessage(QString("✅ Pasted: %1 file(s), Overwritten: %2, Skipped: %3")
        ...
        // ✅ Only reload if something was copied
        if (copiedAny)
    ...
    connect(createFolderButton, &QPushButton::clicked, this, [this]() {
        ...
        if (QDir().mkpath(fullPath)) {
            ...
            dummy.fill(Qt::gray);
            dummy.save(fullPath + "/dummy.jpg");

            statusBar()->showMessage("✅ Created folder: " + folderName, 2000);

            // ✅ Delay UI refresh slightly to allow filesystem to sync
...
void MainWindow::showFolderViewContextMenu(const QPoint& pos) {
    ...
    if (chosen == openAction) {
        ...
        cutMode = true;  // ✅ Mark that we’re cutting, not copying
        ...
        if (copiedAny) {
            ...
            // ✅ Recheck face selection if any are still checked
            for (int f = 0; f < faceList->count(); ++f) {
                QListWidgetItem* faceItem = faceList->item(f);
                if (faceItem->checkState() != Qt::Checked) continue;

                const auto& selectedEmbedding = personList[f].embedding;
            ...
            // Reset static overwrite choice after paste
            applyToAll = false;
            userChoseOverwrite = false;
        }
    }
        ...
        if (ok && !folderName.trimmed().isEmpty()) {
            ...
            if (!QDir(fullPath).exists()) {
                ...
                // ✅ Add dummy image for visibility
                QImage dummy(10, 10, QImage::Format_RGB32);
                dummy.fill(Qt::gray);
                dummy.save(fullPath + "/dummy.jpg");
