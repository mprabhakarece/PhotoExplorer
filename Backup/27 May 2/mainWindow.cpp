...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(faceIndexer, &FaceIndexer::faceLogged, this, [=](const QString& id, const QString& thumbPath) {
...
void MainWindow::refresh() {
    bool wasEmpty = currentPath.isEmpty();
    ...
    if (faceIndexer) {
        if (includeSubfolders) {
            ...
            while (it.hasNext()) {
                ...
                // Save global log after each folder
                QJsonArray updatedGlobal = faceIndexer->getGlobalLog();
                faceIndexer->saveFaceLogAtomic("C:/PhotoExplorer/faces/global_facelog.json", updatedGlobal);
            }
        } else {
    ...
    // Reload UI only if currentPath is valid
    if (!wasEmpty) {
